...
const BarChart = ({data}) => {
    ...
    useEffect(()=>{
        const fd = DataProcessor.parseToCountData(data.records, params.parts);
        setFilteredData(fd);
        draw(fd, params.parts);
            ...
            <div className="params-topbar">
                ...
                <select name="parts" onChange={handleChange}>
                    {Object.keys(PartsEnum).map(
                        (k, i)=><option value={i} key={"parts" + i}>{Parts[i].name}</option>
                    )}
                </select>
            </div>
...
const draw = (data, parts) => {
    ...
    for(let i = 0; i < data.length; i++){
        if(Number(data[i].total_count) > max){
            max = data[i].total_count;
        }
    }
    max = Math.ceil(max / 10) * 10;
    drawDimension(width, height, padding, parts, max);
    drawData(data, parts, width, height, padding, max);
}
const drawDimension = (width, height, padding, parts, max) => {
...
const drawData = (data, parts, width, height, padding, max) => {
    let svg = d3.select('svg');
    ...
    let bardata = [];
    for(let i = 0; i < data.length; i++){
        bardata.push(data[i].total_count);
    }
